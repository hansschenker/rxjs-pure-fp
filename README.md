...
The last feature milestone: the deprecated multicast surface, the remaining
scheduler shapes, the join-all aliases, and every scheduler argument the
earlier milestones had deferred — closed without a class.

```text
ConnectableObservable    branded callable record + connection protocol; stopped subjects are recreated
multicast / refCount     connect-with-selector algebra; RxJS's exact ref-count handshake over the protocol
publish family           multicast over one subject per source application
Scheduler                action factory + clock: createScheduler(createAction, now)
asap / animationFrame    one batch machine over two host edges; a batch closes at flush start
VirtualTimeScheduler     virtual time as a (frame, index)-sorted entry queue; child-action reschedule chains
scheduled                innerFrom's probe order, every case as scheduled work
combineLatestAll/zipAll  toArray → mergeMap(join) → spread projection
```

`scheduled` retires the last recorded deferrals: the trailing-scheduler
forms of `from`/`of`/`range`/`empty`/`pairs`/`generate`/`throwError`/
`startWith`/`endWith`/`concat`/`merge`/`combineLatest`, `delayWhen`'s
`subscriptionDelay`, and the `ReplaySubject`/`shareReplay`/`publishReplay`
time windows over a timestamp provider. `runtime.ts` gains the animation-
frame edge (gate-confined like the timers), and one differential discovery
went back into M13: RxJS closes an asap batch at flush start, so work
admitted mid-flush runs in the *next* microtask — the M13 traces could not
tell the difference, a scheduled async iterable can.

# M18 verification

- **213 / 213 unit** and **314 / 314 differential tests** pass (32 new M18
  traces: ConnectableObservable connect/reset/error/sync-complete, refCount
  operator and method, multicast subject reuse and selector form, the
  publish family, replay windows on a manual clock across
  ReplaySubject/shareReplay/publishReplay, join-all with projections and
  empty sources, every deprecated scheduler form under the queue trampoline,
  scheduled kinds under asap/async incl. microtask interleaving and iterator
  release, the Scheduler factory shape, virtual-time ordering/reschedule
  chains/cancellation/maxFrames/error unwinding/timer+interval/VirtualAction,
  animation-frame batching/rescheduling/cancellation/delays and
  animationFrames);
- RxJS root export parity: **175 / 175 = 100%**; unexpected exports: **0**.

---

# M19 — Package Parity

The package now has RxJS 7.8.2's shape: `main`/`module`/`es2015`/`types`,
an export map with RxJS's conditions in RxJS's order (`types`, `node`,
`require`, `es2015`, `default` — Node and `require` get CommonJS, bundlers
the ES module build), `./package.json`, and the `rxjs/operators` subpath
(115 / 115 names; the seven operator-form names have their own compat
implementations). The two names left on the oracle list, `__esModule` and
`default`, turned out to be exactly what that shape produces: the manifest
was captured by importing `rxjs` through Node, which resolves the `node`
condition to CommonJS and exposes the interop artifacts. The package is
measured through the same door, and `tools/check-package.mjs` gates the
shape plus the agreement of the import, require, ES-module, and declaration
views of every entry. Out of scope and reported as such: `./ajax`,
`./fetch`, `./webSocket` (`./testing` followed in M21).

# M20 — Differential Certification

Export parity is strict (a missing or unexpected export fails the gate), and
certification is derived rather than claimed: `tools/certification-matrix.mjs`
reads the oracle import lists of every differential suite and writes
`docs/CERTIFICATION-MATRIX.md` — every one of the 175 root names and 115
subpath names is traced against the oracle by at least one suite, or the
gate fails. `feature-parity-list.md` is generated by the same tool.

---

# M21 — Marble Testing

`rxjs/testing` was the one oracle subpath whose surface is pure scheduling
rather than host I/O, so it joins the mission after the root closed:
`TestScheduler` as a record composed over the M18 virtual-time machine,
without the subclass.

```text
TestScheduler     virtual-time record + expectation closures; the statics live on the factory
cold / hot        branded Observable function / anonymous Subject, each carrying its subscription log
parseMarbles      pure functions of (diagram, mode, frame factor); emoji-safe code points
expectObservable  subscribe/unsubscribe as scheduled actions; emitted Observables materialized by frame
run               one delegate on the runtime's host edge where RxJS fills six provider slots
animate           frame callbacks queued until the marble fires them
```

RxJS's run mode works by assigning a `delegate` to `intervalProvider`,
`immediateProvider`, `timeoutProvider`, `animationFrameProvider`, and the
two timestamp providers — that is what lets `asyncScheduler`,
`asapScheduler`, and `animationFrameScheduler` run in virtual time without
being passed around. This kernel already routes every host call through the
frozen `timerHost` record in `runtime.ts`, so the functional seam is one
closure-held slot behind it: `installTimerHostDelegate` for the duration of
`run()`, the native host otherwise. Intervals, immediates (asap's batch),
and timeouts (the environments' deferral edge — `defer` now rides the same
record) become virtual actions with RxJS's priority at a shared frame:
immediates before intervals before timeouts. Two small kernel changes fell
out: the virtual scheduler's `maxFrames` is a live policy (`number |
() => number`) instead of a field `run` would have to assign, and the queue
protocol is a spreadable carrier (`virtualTimeProtocol`) so a composed
record is a valid `VirtualAction` host.

# M21 verification

- **228 / 228 unit** and **335 / 335 differential tests** pass (21 new M21
  traces against the oracle's own `TestScheduler`: both marble parsers over
  values, groups, offsets, errors, emoji, run-mode time progression and
  every parser error; cold/hot expectations and subscription logs incl. hot
  offsets, late subscriptions, unsubscribe marbles, array forms, and
  pending expectations across flushes; operators over cold inners; run mode
  with `time`, `delay`, `debounceTime`, `throttleTime`, `interval`, `timer`,
  asap/async/queue priority at a shared frame, interval cancellation,
  `animate` with `animationFrames` and the animation-frame scheduler,
  Observable-of-Observables with `toBe` and `toEqual`, error marbles,
  unhandled consumer errors surfacing from `run` and through
  `config.onUnhandledError`, replay windows and `timestamp` on the virtual
  clock; `frameTimeFactor`/`maxFrames`/`createTime` in and out of run mode,
  direct `VirtualAction` construction on the record, the `maxFrames`
  cut-off, and every construction error message);
- `rxjs/testing` export parity: **3 / 3** (`TestScheduler` plus the two
  CommonJS interop artifacts); root parity unchanged at **175 / 175**,
  unexpected exports **0**.

---

# Root parity after M20
...
Schedulers
  asyncScheduler  asapScheduler  queueScheduler  animationFrameScheduler
  async           asap           queue           animationFrame
  Scheduler       scheduled      animationFrames
  VirtualTimeScheduler          VirtualAction
...
That is **175 / 175 = 100%** of the root export names, plus **115 / 115** of
the `rxjs/operators` subpath and, since M21, **3 / 3** of the `rxjs/testing`
subpath:
...
### M18 — Compat Closure ✅
ConnectableObservable as a branded record with a connection protocol + multicast/refCount/publish family; combineAll/combineLatestAll/zipAll; Scheduler as action factory + clock; asap/animationFrame as one batch machine; virtual time as a sorted entry queue; scheduled carrying every deferred scheduler argument and delayWhen's subscriptionDelay; replay time windows; animationFrames.

### M19 — Package Parity ✅
RxJS-identical export map (`__esModule`/`default` as its CommonJS interop artifacts), CommonJS/ESM/declaration agreement gate, the `rxjs/operators` subpath.

### M20 — Differential Certification ✅
strict export parity; the per-name certification matrix derived from the differential suites.

### M21 — Marble Testing ✅
the `rxjs/testing` subpath: TestScheduler composed over the virtual-time machine, cold/hot records carrying subscription logs, run mode as one delegate on the runtime's host edge.
...
Session 7  M18-M20   ✅ compat closure + gates                → 175/175 (100%)
Session 8  M21       ✅ marble testing (rxjs/testing)         → + testing 3/3
```

All eight sessions are complete (Session 3 re-scoped to close at M14).
Every RxJS 7.8.2 root export, the `rxjs/operators` subpath, and the
`rxjs/testing` subpath are implemented and differentially certified; the
per-name evidence lives in `docs/CERTIFICATION-MATRIX.md` and the parity
table in `feature-parity-list.md`, both generated by `npm run parity:matrix`.
