...
import { createSubscriberWithHooks, subscriberEnv, type Observer, type Subscriber } from './sink.ts';
...
export const createOperatorSubscriber = <T, R>(
  ...
  const operatorSubscriber = createSubscriberWithHooks(observer, {
    // F6: the operator child inherits the destination's runtime environment.
...
export const subscribeOperator = <T>(
  ...
  executeSource(source, operatorSubscriber);
  return operatorSubscriber;
};
